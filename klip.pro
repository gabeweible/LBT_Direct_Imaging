pro klip, obj, cube_folder, use_injection, do_destripe, filter, bin, bin_type, do_hyper, do_annmode, combine_type, klip_fraction, start_frame, end_frame, fill, k_klip, angsep, anglemax, nrings, wr, n_ang, annmode_inout, suffix, ct, do_cen_filter, coadd, rho=rho, theta=theta, contrast=contrast, trial=trial, fs=fs, neg_inj=neg_inj
newline = string(10B)
...
klipframe=medframe
   ...
   if runs eq 4 then begin
      ...
      super_suffix = '/Users/gabeweible/OneDrive/research/HII1348/macbook_' + string(coadd) + '/combined/' + obj + '_bin_' + string(sigfig(bin,1)) + '_type_' + bin_type + '_comb_type_'+combine_type+'_k_klip_'+string(sigfig(k_klip,1))+'_angsep_'+string(sigfig(angsep,4))+'_angmax_'+string(sigfig(anglemax,3))+'_nrings_'+string(sigfig(nrings, 2))+'_nang_'+string(sigfig(n_ang,2)) + '_neg_inj_' + string(neg_inj)
