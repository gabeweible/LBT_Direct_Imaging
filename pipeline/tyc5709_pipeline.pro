pro tyc5709_pipeline, rho=rho, theta=theta, planet_x=planet_x, planet_y=planet_y,$
...
dark_frame_start = 'None';57000; pre-coadd start index
...
;------------------------------[ Start User Input ]---------------------------------
; frame half-size in pixels. Mostly limited by top-right position near the
; cold mask artifact. bottom left constraint is more like 180-px radius.
; 100 is probably all I need to capture all of the detectable disk signal.
...
raw_path = '/Volumes/T7/TYC5709/raw'
darks_path = '/Volumes/T7/TYC5709/darks'
...
; update directory
output_path = '/Users/gweible/OneDrive - University of Arizona/research/protoplanet/gabe_macbook1_new/'
...
truenorth_sx = -1.278-pa_off ; +0.131 or -0.225 deg (from Steve and Jared — OLD)
truenorth_dx = 0.874-pa_off ; + 0.176 or -0.292 deg (Jared 06/20/25)
...
if pre_inj eq 1 then begin
   ...
   if pre_clean eq 1 then begin
		...
		;		debug=1, dark_frame_start=dark_frame_start, max_frames_per_group=250,$
		...
		;		output_path, read_header=1, coadd_type=coadd_type, legacy_mode=0,$
		;		dark_frame_start=0,$
		;		max_frames_per_group=500, debug=0, dark_cube=1,$
		;		min_r_squared=0.9, full_well=4095*0.98,skip_first_read=0,$
		;		skip_last_read=0
	...
	;		darks_filename='tyc5709_darks_cube_1trimmed.fits', vapp=0
