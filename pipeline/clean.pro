pro clean, obj_name, cube_folder, corr_thresh, do_cen_filter
...
for xx = 0, (size(cube))[2] - 1 do begin
   for yy = 0, (size(cube))[2] - 1 do begin
      ...
      if sqrt( ((xx - 250.)^2.) + ((yy - 250.)^2.) ) gt 20 then cube2[xx,yy,*] *= 0.0000001
   endfor
endfor
...
writefits, '~/Desktop/test-med_' + string(ct) +  '_corrthresh.fits', lmed
...
if n_elements(bad_pixels) gt 1 then begin
   ...
   writefits, '~/Desktop/test-bad_pixels_' + string(ct) +  '_corrthresh.fits', left_bad_pixels_cube
