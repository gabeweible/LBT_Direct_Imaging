pro gen_bad_pix_mask, dark, flat, mask_out=mask_out, $
    ...
    if n_elements(hot_sigma) eq 0 then hot_sigma = 2.0
   ...
   ; if n_elements(hp_flat_thresh) eq 0 then hp_flat_thresh = [-0.1, 0.1]  ; [low, high]
    ...
    flat = flat / float(median(double(flat), /double, /even))
    ...
    norm_flat_hp = flat_hp / float(median(double(flat_hp), /even, /double))
    ...
    norm_flat_hp_med = float(median(double(norm_flat_hp), /even, /double))
    norm_flat_hp_std = float(stddev(double(norm_flat_hp), /nan, /double))
    ...
    dark_norm = dark / float(median(double(dark), /even, /double))
    dark_norm_hp = dark_norm - filter_image(dark_norm, median=32, /all_pixels)
    ; re-normalize after HP filter
    norm_dark_norm_hp = dark_norm_hp / float(median(double(dark_norm_hp), /even, /double))
    ...
    norm_dark_norm_hp_med = float(median(double(norm_dark_norm_hp), /even, /double))
    norm_dark_norm_hp_std = float(stddev(double(norm_dark_norm_hp), /nan, /double))
