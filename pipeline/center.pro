pro center, obj_name, cube_folder, do_block_right, do_block_left, do_block_bottom, do_block_top, do_cen_filter, do_cen_median_sub
; 'HII1348', '~/OneDrive/Research/HII1348/testing', 1, 1, 1, 1, 1, 0 for current run (let's try cen_median_sub later)
compile_opt idl2

for runs=1,4 do begin; I think this is the second time that the runs matter (we are now post-split)
   ...
   if do_block_right then obj_cube2[350:499, *, *] *= 0.000001
   if do_block_left then obj_cube2[0:150, *, *] *= 0.000001
   ;if do_block_left then obj_cube2[700:1023, *, *] *= 0.000001
   ;if do_block_right then obj_cube2[0:450, *, *] *= 0.000001
   if do_block_bottom then obj_cube2[*, 0:150, *] *= 0.000001
   if do_block_top then obj_cube2[*,350:499, *] *= 0.000001
   ...
   for ii=0, n_frames-1 do begin
      obj_cube[*,*,ii] = fshift(obj_cube[*,*,ii], 250.-xx, 250.-yy)
