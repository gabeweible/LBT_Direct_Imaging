pro kklip_loop_rdi_final2, kklip_arr=kklip_arr, angsep_arr=angsep_arr, bin_arr=bin_arr
...
kklip_arr=[11, 12, 13, 14]
angsep_arr=[0.265, 0.267, 0.269, 0.271, 0.273, 0.275, 0.275, 0.277, 0.279, 0.281,$
	0.283, 0.285, 0.287, 0.289]

bin_type_arr = ['median']
combine_type_arr = ['median', 'nwadi', 'mean']
filter_arr = [29., 58.];[30., 40., 50., 60., 70., 80., 90.]
n_ang_arr = [2]
bin_arr = [10];I should probably try lower coadds as well!
annmode_inout=[8,34]; nwadi needs 1 extra px on either side?
anglemax_arr = [34., 36., 38., 40., 42., 44.] ; max. parallactic angle between ref. and science frame
...
save, filename=output_path+'/kklip_loop_arrays_rdi_final2.sav',kklip_arr, angsep_arr, filter_arr,$
					...
					foreach angsep, angsep_arr do begin
						...
						foreach k_klip, kklip_arr do begin
							...
							alcor_pipeline, pre_inj=0, neg_inj=0, coadd=coadd,$
								...
								rdi_extra='final2', extra='rdi_final2',$
