pro kklip_loop9, kklip_arr=kklip_arr, angsep_arr=angsep_arr, bin_arr=bin_arr
...
kklip_arr=[4,8,16,32];[4,5,6,7];[4,5,6,7];new array from initial results;[8,9]; partially complete;[2, 3, 4, 5, 6, 7, 8, 9]
angsep_arr=[0.200, 0.225, 0.250, 0.275, 3.00];[0.05, 0.06, 0.07, 0.7, 0.8, 0.9];[0.25/fwhm, 0.5/fwhm, 0.75/fwhm, 1.0/fwhm, 2.0/fwhm, 3.0/fwhm, 3.25/fwhm, 3.5/fwhm, 3.75/fwhm, 4.0/fwhm, 5.0/fwhm]

bin_type_arr = ['median']
combine_type_arr = ['median']
filter_arr = [25., 30., 35., 40., 45., 50.];[30., 40., 50., 60., 70., 80., 90.]
n_ang_arr = [1,2]
bin_arr = [8];I should probably try lower coadds as well!
...
save, filename=output_path+'/kklip_loop_arrays9.sav',kklip_arr, angsep_arr, filter_arr,$
		...
		foreach k_klip, kklip_arr do begin
			...
			print, 'k_klip = ', string(k_klip), newline
				...
				alcor_pipeline, pre_inj=0, neg_inj=0, coadd=coadd, uncert=0, klip=1,$
	 			...
	 			combine_type=combine_type,rdi=0
