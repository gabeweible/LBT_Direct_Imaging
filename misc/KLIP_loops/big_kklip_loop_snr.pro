pro big_kklip_loop_snr, coadd, kklip_arr=kklip_arr, angsep_arr=angsep_arr, bin_arr=bin_arr, extra=extra
;+
; NAME:
;       KKLIP_LOOP_SNR
...
restore, output_path+'/kklip_loop_arrays6.sav'

; 2-digit k_klip
files = FILE_SEARCH(output_path, '/Alcor_right_klip_[0-9][0-9]k*_neg_inj_0.fits', COUNT=filecount)
print, 'Found ', filecount, 'right KLIP FITS files!'

; one-digit k_klip
files2 = FILE_SEARCH(output_path, '/Alcor_right_klip_[0-9]k*_neg_inj_0.fits', COUNT=filecount2)
print, 'Found ', filecount2, 'right KLIP FITS files!'

; 3-digit k_klip
files3 = FILE_SEARCH(output_path, '/Alcor_right_klip_[0-9][0-9][0-9]k*_neg_inj_0.fits', COUNT=filecount3)
print, 'Found ', filecount3, 'right KLIP FITS files!'

; 4-digit k_klip
files4 = FILE_SEARCH(output_path, '/Alcor_right_klip_[0-9][0-9][0-9][0-9]k*_neg_inj_0.fits', COUNT=filecount4)
...
fwhm=13.0773; M-band]
xp = 180.0-1.; back to no pa_off;189.163; approx. x-pixel position of Alcor B (w/pa_off=22.)
yp = 209.0-1.; back to no pa_off;198.093 ; approx. y-pixel position of Alcor B (w/pa_off=22.)
; Note: radius is ~ 21 px for Alcor B = 1.9 lambda/D (!!!)
; Note: r = 223.8 mas
; Alcor is at 25.056 pc from Hipparcos, so this is:
; 5.6 au projected separation!!!! Jupiter is at 5.20 au !!!!
half_cropped_sz = 190.
full_frame = 0
...
WRITE_CSV, strcompress(output_path + '/Alcor_contrast_output_big.csv',/rem), results, all_files
save,filename=strcompress(output_path + '/Alcor_contrast_results_big.sav',/rem),$
	results, all_files
