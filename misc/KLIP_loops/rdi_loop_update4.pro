pro rdi_loop_update4, kklip_arr=kklip_arr, angsep_arr=angsep_arr, bin_arr=bin_arr
...
; Alcor cubes
refcube1 = readfits_fast(strcompress(output_path + '/processed_right/dith1/' + obj + '_' +$
	string(keep_number) + '_peak_thresh_' + string(peak_thresh) + '_stddev_thresh_' +$
	string(stddev_thresh) + '_cube_skysub_cen_clean.fits', /r))

refcube2 = readfits_fast(strcompress(output_path + '/processed_right/dith2/' + obj + '_' +$
	string(keep_number) + '_peak_thresh_' + string(peak_thresh) + '_stddev_thresh_' +$
	string(stddev_thresh) + '_cube_skysub_cen_clean.fits', /r))
...
restore, filename=strcompress(output_path + '/processed_right/dith1/' + obj + '_' +$
...
restore, filename=strcompress(output_path + '/processed_right/dith2/' + obj + '_' +$
	string(keep_number) + '_peak_thresh_' + string(peak_thresh) + $
	'_stddev_thresh_' + string(stddev_thresh) +  '_parang_clean.sav', /r)
					...
					foreach angsep, angsep_arr do begin
						...
						foreach k_klip, kklip_arr do begin
							...
							alcor_pipeline, pre_inj=0, neg_inj=0, coadd=coadd,$
								...
								ref_ang=just_alcor_ref_angles, refcube=just_alcor_refcube
